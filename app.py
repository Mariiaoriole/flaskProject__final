...
def get_data():
    rows_url = config.API_MAIN_URL + 'datasets/1982/rows'
    count_url = config.API_MAIN_URL + 'datasets/1982/count'
...
def prepare_data(data):
    ...
    for element in data:
        if 'Cells' in element and element['Cells'] is not None:
            ...
            if 'Quarter' in cell and 'AveragePerCapita' in cell:
                result.append({
                    'quarter_name': cell['Quarter'],
                    'average_count': cell['AveragePerCapita'],
                    'date': dateparser.parse(cell['Quarter']),
                })
